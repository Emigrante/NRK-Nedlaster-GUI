...
namespace NRKLastNed.Mac.Services
{
    public class YtDlpService
    {
        ...
        public YtDlpService(AppSettings settings)
        {
            ...
            _ffmpegPath = PlatformHelper.GetToolPath("ffmpeg");
        }
        ...
        public bool ValidateTools(out string message)
        {
            ...
            {
                message = $"Finner ikke yt-dlp i: {_ytDlpPath}\nOpprett mappen 'Tools' og legg filene der.\nPå macOS må yt-dlp være kjørbar (chmod +x yt-dlp).";
                LogService.Log($"Mangler verktøy: {_ytDlpPath}", LogLevel.Error, _settings);
                return false;
            }
            if (!File.Exists(_ffmpegPath))
            {
                message = $"Finner ikke ffmpeg i: {_ffmpegPath}\nOpprett mappen 'Tools' og legg filene der.\nPå macOS må ffmpeg være kjørbar (chmod +x ffmpeg).";
        ...
        public async Task DownloadItemAsync(DownloadItem item, IProgress<string> progressText, IProgress<double> progressPercent, CancellationToken token)
        {
            ...
            // For radio, bruk bare audio-download uten video-format arguments
            string args;
            if (item.IsTelevision)
            {
            ...
            {
                // Velg riktig output-mappe basert på TV/Radio (outputFolder já deklarert tidligere)
                if (!_settings.UseSameFolderForBoth && !string.IsNullOrEmpty(_settings.RadioOutputFolder) && !item.IsTelevision)
                {
                    outputFolder = _settings.RadioOutputFolder;
                }
                else if (!_settings.UseSameFolderForBoth && !string.IsNullOrEmpty(_settings.TvOutputFolder))
                {
                    outputFolder = _settings.TvOutputFolder;
                }
